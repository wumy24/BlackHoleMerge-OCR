...
\subsection{Per-Layer Quality--Efficiency Ablation}
We conduct the detailed merge-layer analysis on PaddleOCR-VL-1.6 for a reason: among the
surveyed OCR VLMs it exhibits the widest and most stable K-norm separation between text and
background (the largest text--blank Gap in the main-text cross-model table), making it the
cleanest host for isolating the effect of the merge layer from confounding representation
noise. We stress that the goal here is \emph{not} to search for a single ``optimal'' merge
layer---that question is not by itself meaningful, since the best layer depends on each host's
density profile---but to verify that, wherever the text--background density separation is
pronounced, \ours{} compresses heavily while keeping parsing quality. Merging must also happen
after the density signal has formed: when we attempted to merge at layer~2 or earlier, the
K-norm separation has not yet emerged and the training loss fails to converge, so we only
consider mid/late merge layers.

Table~\ref{tab:supp_layer_full} gives the full quality--efficiency breakdown that the main
text condenses. We report all three candidate merge layers (L3/L6/L9) with both efficiency
(retained tokens, ViT and prefill TFLOPs) and the complete OmniDocBench~v1.6 parsing metrics
(Text Edit, Formula CDM, Table TEDS/TEDS\textsubscript{s}, Reading-Order Edit, and Overall).
Merging earlier (L3) removes the most ViT compute but slightly weakens parsing, whereas a
mid/late layer (L9) attains the best overall quality at a comparable token budget, which is
the configuration reported as \ours{}~(best) in the main text.
...
\bottomrule
\end{tabular*}
\caption{Full per-layer merge ablation on PaddleOCR-VL-1.6 with combined efficiency and
OmniDocBench~v1.6 parsing metrics. The merge layer is swept over three candidates (L3/L6/L9);
the L9 row (bold) is the \ours{}~(best) configuration reported in the main text. Merged is
the retained visual-token count (compression rate in parentheses); ViT/Prefill are TFLOPs.}
\label{tab:supp_layer_full}
\end{table*}

\subsection{Density and Keep-Ratio Analysis}
The per-stage training progression is reported in the main text (the per-stage progression
table in the Experiments section), which lists the full OmniDocBench~v1.6 parsing metrics
along the same six-step sequence---(1) Official, (2) No-merge SFT, (3) No-merge SFT + RL,
(4) Merge-aware SFT, (5) Merge-aware SFT + RL, and (6) Merge-aware SFT + RL + post-SFT
(\ours{}~(best))---under a controlled in-encoder setting: the same initialization, the same
$1\times$ projector, and the same data and evaluation protocol. Two observations from that
table motivate the finer analysis here: our data alone (2) already lifts Overall above the
official E2E scale, while RL without a merge-aware SFT prior (3) degrades quality---especially
table structure---rather than compressing usefully. Below we
break down \emph{why}, through the density and keep-ratio behavior across merge layers and
training stages.
