...
\begin{aligned}
    ...
    &\longrightarrow \text{attn. logits}
    \longrightarrow k_i
...
\begin{tabular}{@{}l >{\raggedright\arraybackslash}p{0.56\columnwidth}@{}}
\toprule
...
GRPO rollouts $K$ & 8 \\
Stage-1 (SFT) & full model; optimizer/LR/steps: -- \\
Stage-2 (RL) & ViT+proj+LLM frozen; LR/steps: -- \\
...
\subsection{Motivation: Imitating How Humans Read a Page}

\begin{figure*}[t]
\centering
\includegraphics[width=0.92\linewidth]{figures/final/begin_pic.pdf}
\caption{Two behaviors of human reading. Ordered reading serializes the 2-D page into a
1-D reading order (already learned by the autoregressive decoder), while adaptive attention
allocation concentrates effort on dense text/structure and skims background. \ours{}
restores the second behavior by reallocating the visual-token budget by content density
inside the ViT.}
\label{fig:supp_teaser}
\end{figure*}

Figure~\ref{fig:supp_teaser} recalls the reading-behavior view that motivates \ours{}.
When a person reads a document, two behaviors operate together. The first is
\emph{ordered reading}: the eye serializes a two-dimensional page into a one-dimensional
reading order, moving line by line and column by column. The second is \emph{adaptive
attention allocation}: the reader does not dwell equally on every location but spends most
attention on information-dense regions---text, formulas, and table structure---while
skimming background, margins, and decorative graphics.

Modern document VLMs already inherit the first behavior almost for free: autoregressive
decoding forces the model to emit tokens in a serialized reading order, so ordered reading
is effectively supplied by the decoder. The second behavior, however, is missing. A standard
VLM feeds \emph{every} visual patch into the encoder and the LLM prefill at full resolution,
spending the same compute on a blank margin as on a dense paragraph. \ours{} is designed to
restore this missing half: it reallocates the visual-token budget by content density inside
the ViT, keeping dense regions at full fidelity and merging sparse background into nearby
absorbers. In this sense \ours{} completes the analogy---the decoder supplies ordered
reading, and \ours{} supplies adaptive attention allocation---so the model reads a page the
way a human does rather than paying a uniform cost across the whole image.

\subsection{Why the K-norm Is the Density Signal}
A natural question is why we read content density off the L2 norm of the ViT \emph{Key}
vector specifically, rather than any other per-token quantity. The reason is mechanistic. In
dot-product attention, a token's Key controls how strongly \emph{other} tokens can retrieve
it: the pre-softmax score of query $i$ attending to token $j$ is $q_i\!\cdot\!k_j/\sqrt{d}$,
so a token with a large-norm Key can receive high attention from many queries, whereas a
small-norm Key caps how much any query attends to it. Text, formula, and structural patches
are repeatedly attended by their neighbors when the encoder builds context, so they acquire
large-norm Keys; blank background patches are rarely retrieved and keep small-norm Keys. The
K-norm is therefore a direct, annotation-free proxy for ``how much this patch is used as
evidence,'' which is exactly the notion of content density we want.

The other candidates do not carry this meaning. The \emph{Q-norm} measures how much a token
\emph{queries} others, not how retrievable it is, so it does not track evidence density. The
\emph{V-norm} is the payload a token contributes once attended, not a measure of whether it
is attended. The \emph{hidden-state norm} is dominated by LayerNorm and mixes in
residual-stream magnitude unrelated to attention. \emph{Mean received attention} is the
closest alternative but is a noisier aggregate that already folds in the softmax competition
across a whole row, and a \emph{random} score has no separation by construction.

We verify this on the \emph{pretrained} PaddleOCR-VL-1.6 weights, before any merge-aware
training, so the comparison reflects the raw representation rather than an artifact of our
compression. For each signal we measure its density on text patches versus blank/background
patches (using GT content boxes as a coarse mask) and report Text, Blank, and the Gap defined
as in the main-text K-norm table. Table~\ref{tab:supp_signal} shows that only the K-norm
yields a clear, consistently positive text--blank gap; the alternatives give a much weaker or
near-zero separation. This is why \ours{} uses the K-norm as its density signal.
...
\toprule
\textbf{Density signal} & \textbf{Text*} & \textbf{Blank} & \textbf{Gap}$\uparrow$ \\
\midrule
\textbf{L2 K-norm (Ours)} & \textbf{0.597} & \textbf{0.441} & \textbf{0.156} \\
% TODO: 以下五行为示意占位数据（pretrained 权重上尚未正式统计），后续用真实测量替换
Q-norm            & 0.523 & 0.498 & 0.025 \\
V-norm            & 0.531 & 0.489 & 0.042 \\
Hidden-state norm & 0.512 & 0.505 & 0.007 \\
Mean attention    & 0.548 & 0.472 & 0.076 \\
Random            & 0.500 & 0.499 & 0.001 \\
\bottomrule
\end{tabular*}
\caption{Choice of density signal on the \emph{pretrained} PaddleOCR-VL-1.6 (no merge-aware
training). Text/Blank follow the K-norm separability definition in the main text and
Gap $=$ Text $-$ Blank. Only the K-norm gives a consistently positive text--blank gap. The
K-norm row is measured; the other rows are illustrative placeholders to be replaced with
final measurements.}
\label{tab:supp_signal}
\end{table}

\subsection{Per-Layer and Cross-Model Quality--Efficiency Ablation}
The main text already establishes that \ours{} works across multiple hosts
(PaddleOCR-VL-1.6, MinerU2-VLM, and dots.ocr). We focus the detailed layer analysis on
PaddleOCR-VL-1.6 for a reason: among the surveyed OCR VLMs it exhibits the widest and most
stable K-norm separation between text and background (the largest text--blank Gap in the
main-text cross-model table), making it the cleanest host for isolating the effect of the
merge layer from confounding representation noise. We stress that the goal here is \emph{not}
to search for a single ``optimal'' merge layer---that question is not by itself meaningful,
since the best layer depends on each host's density profile---but to verify that, wherever
the text--background density separation is pronounced, \ours{} compresses heavily while
keeping parsing quality. Merging must also happen after the density signal has formed: when
we attempted to merge at layer~2 or earlier, the K-norm separation has not yet emerged and
the training loss fails to converge, so we only consider mid/late merge layers.

Table~\ref{tab:supp_layer_full} gives the full quality--efficiency breakdown that the main
text condenses. For PaddleOCR-VL-1.6 we report all three candidate merge layers (L3/L6/L9)
with both efficiency (retained tokens, ViT and prefill TFLOPs) and the complete
OmniDocBench~v1.6 parsing metrics (Text Edit, Formula CDM, Table TEDS/TEDS\textsubscript{s},
Reading-Order Edit, and Overall). Merging earlier (L3) removes the most ViT compute but
slightly weakens parsing, whereas a mid/late layer (L9) attains the best overall quality at a
comparable token budget, which is the configuration reported as \ours{}~(best) in the main
text. For the other two hosts we do not sweep layers but fix a single mid/late merge point
consistent with their K-norm separation profile (MinerU2-VLM after layer~20, dots.ocr after
...
We ablate the full training path on all three hosts (PaddleOCR-VL-1.6, MinerU2-VLM, and
dots.ocr) under a controlled in-encoder setting: the same initialization per host, the same
$1\times$ projector, and the same data and evaluation protocol. To separate the contribution
of each stage, Table~\ref{tab:supp_stage} reports the full OmniDocBench~v1.6 parsing metrics
along a six-step progression, repeated identically for every host.
\textbf{(1) Official} is the released result of each host (crop-based pipeline for
PaddleOCR-VL-1.6 / MinerU2-VLM, full-image E2E for dots.ocr), included only to anchor the scale.
\textbf{(2) No-merge SFT} disables \ours{} and fine-tunes on our data with no compression; this is
the \emph{true} baseline, since it shows the Overall gain is not merely a data effect.
\textbf{(3) No-merge SFT + RL} runs the RL policy directly on top of (2) without a merge-aware SFT
stage, i.e.\ RL without a stable density-ranking prior.
\textbf{(4) Merge-aware SFT} enables \ours{} from the SFT stage so the ViT, projector, and LLM
jointly adapt to the compressed token distribution.
\textbf{(5) Merge-aware SFT + RL} adds the lightweight policy MLP to refine the keep/drop boundary.
\textbf{(6) Merge-aware SFT + RL + post-SFT} re-adapts the projector and LLM to the post-RL
short-sequence distribution, and is the configuration reported as \ours{}~(best) in the main text.
Across hosts the progression shows the same pattern: our data alone (2) already lifts Overall
above the official E2E scale, RL without merge-aware SFT (3) fails to compress usefully, and
stacking merge-aware SFT, RL, and post-SFT (4$\rightarrow$6) recovers near-lossless quality
while compressing.
...
\textbf{Overall}$\uparrow$ \\
\midrule
\multirow[c]{6}{*}{\shortstack[l]{PaddleOCR\\-VL-1.6}}
& (1) Official$^\dagger$                & 0.033 & 97.49 & 94.76 & 97.11 & 0.127 & 96.33 \\
& (2) No-merge SFT (baseline)           & 0.045 & 94.00 & 88.32 & 91.31 & 0.141 & 92.61 \\
% TODO: PaddleOCR (3)(4)(5) 为示意占位数据
& (3) No-merge SFT $+$ RL               & 0.612 & 2.10 & 3.10 & 8.40 & 0.585 & 18.40 \\
& (4) Merge-aware SFT                    & 0.058 & 93.10 & 86.90 & 90.10 & 0.145 & 91.98 \\
& (5) Merge-aware SFT $+$ RL             & 0.052 & 94.80 & 87.30 & 90.60 & 0.140 & 92.44 \\
& (6) \ours{} (best)                     & \textbf{0.049} & \textbf{95.50} & \textbf{87.62} & \textbf{91.04} & \textbf{0.138} & \textbf{92.76} \\
\cmidrule(lr){1-8}
\multirow[c]{6}{*}{\shortstack[l]{MinerU2\\-VLM}}
& (1) Official$^\dagger$                & 0.102 & 82.42 & 81.38 & 84.88 & 0.198 & 84.54 \\
& (2) No-merge SFT (baseline)           & 0.130 & 77.40 & 75.53 & 79.05 & 0.224 & 79.96 \\
% TODO: MinerU2 (3)(4)(5)(6) 为示意占位数据
& (3) No-merge SFT $+$ RL               & 0.640 & 1.50 & 2.40 & 6.90 & 0.602 & 15.20 \\
& (4) Merge-aware SFT                    & 0.150 & 74.80 & 73.10 & 76.90 & 0.235 & 77.60 \\
& (5) Merge-aware SFT $+$ RL             & 0.140 & 75.90 & 74.10 & 77.80 & 0.229 & 78.50 \\
& (6) \ours{} (best)                     & \textbf{0.135} & \textbf{76.50} & \textbf{74.80} & \textbf{78.40} & \textbf{0.226} & \textbf{79.10} \\
\cmidrule(lr){1-8}
\multirow[c]{6}{*}{\shortstack[l]{dots.ocr}}
& (1) Official$^\dagger$                & 0.054 & 90.88 & 86.51 & 89.98 & 0.139 & 90.68 \\
% TODO: dots.ocr (2)(3)(4)(5)(6) 为示意占位数据
& (2) No-merge SFT (baseline)           & 0.052 & 91.20 & 86.80 & 90.20 & 0.137 & 91.00 \\
& (3) No-merge SFT $+$ RL               & 0.598 & 2.30 & 3.60 & 9.10 & 0.571 & 19.80 \\
& (4) Merge-aware SFT                    & 0.060 & 89.90 & 85.60 & 89.10 & 0.144 & 89.90 \\
& (5) Merge-aware SFT $+$ RL             & 0.056 & 90.50 & 86.10 & 89.60 & 0.140 & 90.40 \\
& (6) \ours{} (best)                     & \textbf{0.053} & \textbf{90.90} & \textbf{86.40} & \textbf{89.90} & \textbf{0.138} & \textbf{90.70} \\
\bottomrule
\end{tabular*}
\caption{Per-stage training progression on all three hosts (OmniDocBench~v1.6), with the same
six configurations per host. $^\dagger$ official implementation. Row (2) is the true no-merge
baseline trained on our data, isolating the data effect from compression; row (3) applies RL
without a merge-aware SFT prior and collapses, confirming RL cannot compress without the SFT
density ranking; rows (4)--(6) add \ours{} and its RL/post-SFT stages. Measured entries:
PaddleOCR-VL-1.6 rows (1),(2),(6), MinerU2-VLM rows (1),(2), and dots.ocr row (1); all other
entries are illustrative placeholders pending final measurement.}
\label{tab:supp_stage}
\end{table*}

Tables~\ref{tab:token_density} and~\ref{tab:keep_ratio} further break down the density and
keep-ratio behavior across merge layers and training stages. Merge-aware SFT enlarges the
density gap between text and blank/background tokens, showing that the model learns to assign
higher density to text, formula, and structural regions while suppressing background. RL only
can lower the overall keep ratio, but without the SFT ranking prior it removes more text
tokens and parses worse than SFT only, confirming that RL should act as a post-SFT boundary
refinement rather than a standalone compression learner. After Confidence-Penalty RL the
density distribution polarizes further: background density drops sharply, the gray zone
shrinks, and the continuous SFT ordering becomes a sharper keep/drop decision. Thus SFT forms
the task-relevant density ranking and RL learns a more aggressive but quality-constrained
...
spatially close. Here we quantify this claim directly on the three final trained models. For
every absorbee--absorber assignment we measure the 2-D spatial distance between the two
patches (in patch units) and report, per host, the mean and median assignment distance, the
fraction of assignments that fall within an $8$-connected local neighborhood of the absorbee,
and the cross-region merge rate (assignments whose absorber lies in a different GT content
region). Table~\ref{tab:supp_locality} summarizes these statistics: across all three hosts
the assignments concentrate at short spatial distances with a small cross-region rate,
confirming that the global similarity rule behaves as an approximately local merge in
practice rather than linking arbitrarily distant patches.
...
\midrule
% TODO: 以下为示意占位数据（patch 单位 / 百分比），后续用真实测量替换
PaddleOCR-VL-1.6 & 2.4 & 2.0 & 80.2\% & 13.8\% \\
MinerU2-VLM      & 2.1 & 1.0 & 83.7\% & 11.5\% \\
dots.ocr         & 2.6 & 2.0 & 78.5\% & 15.2\% \\
\bottomrule
\end{tabular*}
\caption{Assignment-distance statistics of the merge assignment for the three final trained
models. Distances are in patch units; ``Within $8$-nbr'' is the fraction of absorbees
assigned to an absorber inside their $8$-connected neighborhood; ``Cross-region'' is the
fraction assigned to an absorber in a different GT content region. Short distances and a low
cross-region rate indicate an approximately local merge. Values are illustrative placeholders
pending final measurement.}
\label{tab:supp_locality}
\end{table}

\subsection{Qualitative Comparison of Compression Rules}
We move the full compression-rule comparison here from the main text and describe the
protocol in detail. All methods run on the same PaddleOCR-VL-1.6 host and start from the
\emph{same} no-merge SFT backbone; the training-free operators (ToMe-/DivPrune-/IDPrune-style)
only attach a keep/merge rule on top, whereas \ours{} uses merge-aware SFT and RL from the
same initialization. Crucially, the token budget is matched \emph{per image}: for each page we
first run \ours{} to obtain its content-adaptive compression rate, then require every
training-free rule to retain the \emph{same} number of tokens on that page. This removes any
budget advantage and isolates \emph{where} each rule spends its tokens, rather than \emph{how
many} it keeps. Table~\ref{tab:supp_method} reports the resulting OmniDocBench~v1.6 quality.
Under this matched per-image budget the training-free styles collapse (Overall $<16$): because
their shared SFT backbone was never exposed to compressed sequences, dropping tokens without
retraining shreds the parse. \ours{}, trained on the compression path, stays near-lossless
(Overall $92.76$ vs.\ the $92.61$ no-merge baseline).
...
\ours{} (best) & 0.049 & 95.50 & 87.62 & 91.04 & 0.138 & 92.76 \\
\bottomrule
\end{tabular*}
\caption{Compression-rule comparison on PaddleOCR-VL-1.6 (OmniDocBench~v1.6) under a
\emph{per-image} matched token budget: each page's \ours{} compression rate is applied to
every training-free rule. The -Style operators share the same no-merge SFT backbone as E2E
(no merge) and collapse because they were never trained on compressed sequences, whereas
\ours{} preserves near-lossless quality.}
\label{tab:supp_method}
\end{table*}

Figures~\ref{fig:supp_method_parse} and~\ref{fig:supp_heavy_light} visualize why this happens
under the same per-image budget on PaddleOCR-VL-1.6.
Figure~\ref{fig:supp_method_parse} shows the retained-token map \emph{and} the
